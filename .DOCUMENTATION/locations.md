...
Dead drop zones are in `custom/configs/locations.lua`. 
Two delivery methods: `find_object` for small packages, `search_vehicle` for big shipments.

Each location has a zone (where players search), spawns (where the package/vehicle appears), and a label.

Add as many locations as you want per method. Players get a random one each order.
